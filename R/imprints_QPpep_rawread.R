...
imprints_QPpep_rawread <- function(data, treatment, contaminant = "Cont"){
  ...
  if(inherits(data, "character")){
    ...
    if(grepl("\\.txt$", data)){
      ...
      data <- openxlsx::read.xlsx(data, check.names = FALSE, sep.names = " ")
    }
    else if(grepl("\\.csv$", data)){
      data <- readr::read_csv(data, check.names = FALSE)
    }
  }
  ...
  colnames(data)[3:(ncol(data)-3)] <- paste0("26C_", colnames(data)[3:(ncol(data)-3)])
