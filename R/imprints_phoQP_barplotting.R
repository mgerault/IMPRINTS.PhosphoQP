...
#'
#' @param data Dataframe obtained after \code{imprints_phoQP_join}
...
imprints_phoQP_barplotting <- function (data, treatmentlevel = get_treat_level(data),
  ...
  if(!("id" %in% colnames(data))){
    cn_needed <- c("Annotated.Sequence", "Master.Protein.Accessions", "Positions.in.Master.Proteins")
    if(all(cn_needed %in% colnames(data))){
      if(NA %in% data$Phosphorylation){
        data$Phosphorylation[which(is.na(data$Phosphorylation))] <- ""
      }
      ids <- paste(paste(data$Annotated.Sequence, data$Modifications),
                   data$Phosphorylation,
                   paste(data$Master.Protein.Accessions, data$Positions.in.Master.Proteins),
      ...
      stop(paste0("If you don't have the column 'id' in your data, you need at least the columns
                  Annotated.Sequence, Master.Protein.Accessions and Positions.in.Master.Proteins
